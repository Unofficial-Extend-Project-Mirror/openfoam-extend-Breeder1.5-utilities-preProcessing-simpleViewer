...
void blockDescriptor::setEdge(label edgeI, label start, label end, label dim)
{
    ...
    pointField blockPoints(blockShape_.size());
    forAll(blockPoints, i)
    {
	blockPoints[i] = blockMeshPoints_[blockShape_[i]];
    }
    ...
    if (!found)
    {
      ...
      */
      ...
      List<point> tmp(2);
      tmp[0] = blockMeshPoints_[blockShape_[start]];
      tmp[1] = blockMeshPoints_[blockShape_[end]];

      edgePoints_[edgeI] = tmp;
      //edgeWeights_[edgeI] = divEdge.lambdaDivisions();
    }
}
